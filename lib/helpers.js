...
helpers.noCoverage = !!grunt.option('no-coverage');
helpers.withCoverage = function(task, alternative) {
  if (typeof alternative === 'undefined') {
    alternative = '';
  }
  return (helpers.noCoverage ? alternative : task);
};

var noJshint = !!grunt.option('no-jshint');
helpers.withJshint = function(task, alternative) {
  if (typeof alternative === 'undefined') {
    alternative = '';
  }
  return (noJshint ? alternative : task);
};
...
helpers.getJshintRc = function() {
  ...
  /* allow true for nested .jshintrc files and false for no .jshintrc */
  if (jshintrcFile === true || jshintrcFile === false) {
    return jshintrcFile;
  }
...
function getStyles() {
  ...
  resolveAssets(getFiles('src.sass')).forEach(function(file) {
    if (path.basename(file)[0] !== '_') {
      styles += tag.replace(':href', file.replace('.scss', '.css'));
    }
  });
