...
files.src.sass = getProjectConfig('files.src.sass', [
  'src/sass/**/*.scss'
]);

files.src.sassFolder = getProjectConfig('files.src.sassFolder', 'src/sass/');
...
demoEnvironmentE2e = demoEnvironmentE2e.map(function(file) {
  if (file.indexOf('src/js') === 0) {
    if (!helpers.noCoverage) {
      file = file.replace('src/js', '.tmp/instrumented/src/js');
    }
    return file;
  }
  return file;
